...
def save(i):
    ...
    np.savetxt( os.path.join(save_dir, 'prediction'), prediction, header='kappa: {}'.format(kappa), fmt='%d' )
...
def predict(i):
    ...
    prediction=generate_text_FB( input_x[:1000], 400000 )
...
# Read data
# ----------------------------------------------- #
infile = '../DATA/Train/AIB9/idx_sum_phi.txt'
input_x = np.loadtxt(infile, unpack=True, dtype='int')
